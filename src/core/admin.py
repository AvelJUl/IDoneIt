...
from core.models import Role, Project, Permission
...
class PermissionInline(_admin.StackedInline):
    model = Permission
    fields = ['member', 'role']

    def get_queryset(self, request):
        qs = super(PermissionInline, self).get_queryset(request)
        return qs.filter(user_set__created_by_id=request.user.id, role_set__created_by_id=request.user.id)
...
class ProjectAdmin(_admin.ModelAdmin):
    ...
    def account_actions(self, obj):
        return format_html(
            ...
            <a href="{}" class="btn btn-info">Edit</a>&nbsp;\
            <a href="{}" class="btn btn-danger">Delete</a>',
            reverse('admin:core_project_change', args=[obj.pk]),
            reverse('admin:core_project_change', args=[obj.pk]),
            reverse('admin:core_project_delete', args=[obj.pk]),
        )
